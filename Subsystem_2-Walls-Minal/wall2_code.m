...
beq = [];
nonlcon = [];
fun = @(x)(2*z*X)*(T1-T2)*dt/((x(3)/x(1))+(x(4)/x(2))+(x(5)/x(1))+(1/h1)+(1/h2)+(outershell_thickness/outershell_k));
...
for i = 1:mat1_recordnumber(1);
    for j = 1:mat2_recordnumber(1);        
        ...
        min_thickness = (total_ceiling_force*0.5)/( 2 * X * m1_yieldstress(i)); %multiplied by 0.5 as wall1 supports 50% of weight 
        lb = [min_thickness,0.001,0.01] ;         
        x0 = [min_thickness+0.05,0.1,0.1]  ;    %offset X0 by 0.05
        %% function
        fun = @(x)(2*z*X)*(T1-T2)*dt/((x(1)/k_1)+(x(2)/k_2)+(x(3)/k_1)+(1/h1)+(1/h2)+(outershell_thickness/outershell_k));       
        %% running IP and SQP        
...
for i = 1:mat1_recordnumber(1);
    for j = 1:mat2_recordnumber(1);       
        ...
        k_2 = m2_conductivity(j);             
        %% min thickness of L1 based on compressive strength
        min_thickness = (total_ceiling_force*0.5)/( 2 * X * m1_yieldstress(i)); 
        lb = [min_thickness,0.001,0.01] ;         
...
for i = 1:mat1_recordnumber(1);
    for j = 1:mat2_recordnumber(1);       
        ...
        k_2 = m2_conductivity(j);             
        %% min thickness of L1 based on compressive strength 
        min_thickness = (total_ceiling_force*0.5)/( 2 * X * m1_yieldstress(i)); %change for wall2
        lb = [min_thickness,0.001,0.01] ;         
        x0 = [min_thickness+0.05,0.1,0.1];       
        %% function
        fun = @(x)(2*z*X)*(T1-T2)*dt/((x(1)/k_1)+(x(2)/k_2)+(x(3)/k_1)+(1/h1)+(1/h2)+(outershell_thickness/outershell_k));        
...
row = row(1);
for v = 1:row
    tot_cost_l1 = z*X*combo_data(v,3)*price_density_data(v,3)*price_density_data(v,1) ;%vol x density x cost
    tot_cost_l2 = z*X*combo_data(v,4)*price_density_data(v,4)*price_density_data(v,2) ;%vol x density x cost
    tot_cost_l3 = z*X*combo_data(v,5)*price_density_data(v,3)*price_density_data(v,1); %vol x density x cost
...
q_no_insulation = ((2*z*X)*(T1-T2))/(outershell_thickness/outershell_k);
